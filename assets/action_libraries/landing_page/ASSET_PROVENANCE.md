...
Action IDs and wrapper paths remain stable. `standard_walk_start` and
`standard_walk_stop` remain legacy compatibility members for now.
`wave_greeting` and `stairs_up_loop` remain unchanged because Character1 has no
